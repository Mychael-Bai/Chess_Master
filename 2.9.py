...
class MCTS:
    ...
    def evaluate_move(self, move, board):
        ...
        if move_gives_check(move, board):
            score += 50
            
        # Check if move restricts king mobility
        if reduces_king_mobility(move, board):
            score += 20
            
        # Check if move controls key squares
        if controls_key_squares(move, board):
            score += 15
    ...
    def get_promising_moves(self, board):
        ...
        # 1. First check for immediate checkmate
        mate_moves = find_checkmate_moves(board)
        if mate_moves:
            return mate_moves
            
        # 2. Check for moves that give check
        check_moves = find_check_moves(board)
        moves.extend(check_moves)
        
        # 3. Look for moves that restrict king mobility
        trap_moves = find_king_trap_moves(board)
        moves.extend(trap_moves)
        
        # 4. Add moves that control key squares
        moves.extend(find_control_moves(board))
