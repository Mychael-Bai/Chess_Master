...
class MCTSNode:
    ...
    def _is_in_check(self, color):
        # Copy of is_in_check from ChineseChess class
        ...
        # First check facing generals
        if self._is_generals_facing(red_king_pos, black_king_pos):
            return True
    ...
    def _is_generals_facing(self, red_king_pos, black_king_pos):
        ...
        for row in range(min(red_row, black_row) + 1, max(red_row, black_row)):
        ...
        for row in range(10):
            for col in range(9):
                ...
                if piece and piece[0] == attacking_color[0].upper():
                    # Pass False to avoid recursive check validation
                    if self._is_valid_move((row, col), pos, check_for_check=False):
                        return True
        return False
    ...
    def _is_valid_move(self, from_pos, to_pos, check_for_check=True):
        ...
        # Only check for check when explicitly requested (avoid recursion)
        if check_for_check:
...
class MCTS:
    ...
    def __init__(self, game_state, simulation_limit=50):
        self.root = MCTSNode(
            board=[row[:] for row in game_state.board],
            current_player=game_state.current_player
        )
        self.simulation_limit = simulation_limit
        self.max_depth = 10  # Add depth limit
    ...
    def make_move(self):
        ...
        in_check = self.root._is_in_check(self.root.current_player)
        if in_check:
            # Try to find moves that escape check
            for child in self.root.children:
                # Make the move temporarily
                from_pos, to_pos = child.move
                piece = self.root.board[from_pos[0]][from_pos[1]]
                original = self.root.board[to_pos[0]][to_pos[1]]
                
                self.root.board[to_pos[0]][to_pos[1]] = piece
                self.root.board[from_pos[0]][from_pos[1]] = None
                
                still_in_check = self.root._is_in_check(self.root.current_player)
                
                # Restore board
                self.root.board[from_pos[0]][from_pos[1]] = piece
                self.root.board[to_pos[0]][to_pos[1]] = original
                
                if not still_in_check:
                    return child.move
        
        # If not in check or no escape moves found, choose best move
    ...
    def simulate(self, node):
        ...
        moves_count = 0
        max_moves = self.max_depth  # Use depth limit instead of 100

        # Try to get out of check if in check
        in_check = node._is_in_check(current_player)
        if in_check:
            valid_moves = []
            for row in range(10):
                for col in range(9):
                    ...
                    if piece and piece[0] == current_player[0].upper():
                        for to_row in range(10):
                            for to_col in range(9):
                                ...
                                if node._is_valid_move(move[0], move[1]):
        ...
        while moves_count < max_moves:
            ...
            # If in check, only consider moves that get out of check
            for row in range(10):
                for col in range(9):
                    piece = board[row][col]
                    if piece and piece[0] == current_player[0].upper():
                            ...
                            for to_col in range(9):
                                ...
                                if node._is_valid_move(move[0], move[1]):
                                    if in_check:
                                        ...
                                        board[row][col] = None
                                        still_in_check = node._is_in_check(current_player)
                                        board[row][col] = piece
        ...
        # Evaluate final position with piece values and check status
        black_score = 0
        red_score = 0
        for row in board:
            for piece in row:
                if piece:
                    ...
                    if piece[0] == 'B':
                        black_score += value
                        if node._is_in_check('red'):
                            black_score += 5
                    else:
                        red_score += value
                        if node._is_in_check('black'):
                            red_score += 5
                            
        return 1 if black_score > red_score else 0
...
class ChineseChess:
    ...
    def handle_game_end(self):
        ...
        self.current_replay_index = len(self.move_history)  # Set to end of game
        self.draw_board()  # Redraw the final position
                ...
                # If clicking on a valid move position
                    ...
                    if self.is_in_check(self.current_player):
                        ...
                        if self.is_in_check(self.current_player) and self.is_checkmate(self.current_player):
                            ...
                            if self.current_player == 'red':
                                self.show_centered_warning("游戏结束", "黑方胜利！")
                            else:
                                self.show_centered_warning("游戏结束", "红方胜利！")
                            return
    ...
    def make_ai_move(self):
        ...
        try:
            ...
            max_time = 3  # Reduce from 5 seconds to 3
            
            # Try to find a good move within time limit
            best_move = None
            while time.time() - start_time < max_time:
                move = mcts.make_move()
                if move:
                    best_move = move
                    # Break early if we find a good move
                    if mcts.root.children and max(mcts.root.children, key=lambda c: c.visits).visits > 10:
                        break
