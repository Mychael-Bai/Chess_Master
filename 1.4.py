...
class ChineseChess:
    ...
    def copy_game_state(self):
        ...
        new_state = ChineseChess.__new__(ChineseChess)  # Create without __init__
        ...
        # Initialize necessary attributes without sound
        new_state.board_size = self.board_size
        new_state.cell_size = self.cell_size
        new_state.piece_radius = self.piece_radius
        new_state.board_margin = self.board_margin
        new_state.canvas_width = self.canvas_width
        new_state.canvas_height = self.canvas_height
        
        # Initialize empty lists and None values for other attributes
        new_state.move_history = []
        new_state.replay_mode = False
        new_state.current_replay_index = 0
        new_state.saved_board_states = []
        new_state.selected_piece = None
        new_state.highlighted_positions = []
        new_state.game_history = []
        
        # Skip sound and UI elements in the copy
        new_state.move_sound = None
        new_state.window = None
        new_state.canvas = None
        new_state.main_frame = None
        new_state.board_frame = None
        new_state.button_frame = None
        new_state.restart_button = None
        new_state.replay_button = None
        new_state.prev_move_button = None
        new_state.next_move_button = None
